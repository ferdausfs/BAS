...
## ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
## BAS0001 — PREMIUM SOFT-PINK REDESIGN — Phase 5 (account/admin/splash) ✅ (2026-07-19, arena.ai Agent Mode)
## ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
**Agent/Tool:** arena.ai Agent Mode (codename BAS0001)
**Phase worked on:** **Phase 5 — Account / Admin / Splash (batch D).** Phase 4 post-purchase was completed and confirmed on `main` before work (newest log entry + commit `60e60eb`). **NEXT RUN = Phase 6 (final consistency pass across ALL screens — side-by-side spacing/radius/shadow/type audit + repo-wide `grep -rn` for leftover `coral`-era references… note: `coral` itself is the intentional primary token name since Phase 0 — hunt for `glass-*`, `backdrop-blur`, `Fraunces`, `Great Vibes`, inline cocoa hexes, `bg-gradient`, `font-display`/`font-brand` stragglers only; plus swipe-gesture + fixed-overlay regression check per section 0 of the brief).** One phase only; no other phase's files were edited.

### কী বদলেছে — premium soft-pink account/admin/splash batch (exact Phase-5 source files)
- **`src/screens/SplashScreen.tsx`** (full visual rebuild, logic untouched): Page bg `bg-[#FBF6EF]` → `bg-bg`. Hero card stripped of cocoa `linear-gradient(145deg,#6B3A18,#3D2418)` + `blur-xl` blob + `backdrop-blur` inner tile → solid `bg-secondary rounded-[32px] shadow-card` with a white `bg-surface shadow-card` emoji tile resting on it (real elevation, depth ≠ hue), plus two quiet solid decorative circles (no blur anywhere). Tag chip `bg-amber-900/10` → `bg-secondary`. `font-display` removed (Poppins default). Pagination dots: active `bg-[#5C3A22]` → `bg-coral`, inactive `bg-ink-100` → `bg-coral-200`. CTA keeps `btn-primary`, `shadow-lg` → `shadow-btn`.
- **`src/screens/ProfileScreen.tsx`** (all logic — geolocation, Firestore profile save, referral claim, address/date CRUD, 5-tap admin shortcut, ChatBot embed, `useModalDepth` hooks — untouched):
  - **Header → QuickBar-safe**: `px-5 pt-3` → `px-5 pr-18 pt-6 pb-3` (Phase-4 OrdersScreen pattern) + new `text-primary` "Account" eyebrow + 24px title; `font-display` dropped. QuickBar (fixed `top-4 right-4` z-45) no longer competes with the title row.
  - **Guest state** → designed empty state: `bg-secondary` coral-icon circle + headline + copy + `bg-coral shadow-btn` CTA (was bare icon + flat button).
  - **Identity card**: dark ink-gradient + `backdrop-blur` avatar tile + `bg-white/20` chip → white `bg-surface border-border shadow-card rounded-[24px]` card, solid `bg-coral shadow-btn` avatar tile, `bg-secondary text-coral` member chip, soft `bg-secondary`/`bg-accent/40` decorative circles (solid, no blur).
  - **Wallet card**: cocoa gradient (`from-[#5C3A22] via-[#3D2418]`) + cocoa rgba shadow → **solid `bg-coral` brand-pink money card** (bKash-pink convention) with `shadow-btn` and translucent white decorative/history-row tints (no blur).
  - **Stats/shortcuts/menu**: all `glass-strong` + inline cocoa rgba shadows → `border-border bg-surface shadow-card rounded-[20px]`; icon tiles `bg-ink-50 text-ink-200` → `bg-secondary text-coral`; dividers `border-ink-50` → `border-border`; wishlist rail thumbnails get `border-border shadow-card`; "See all" → `text-coral`.
  - **Invite card**: `border-dashed border-ink-100` + `bg-ink` icon/button → coral-tinted dashed promo card (`border-coral/40 bg-secondary/50 shadow-card`) with `bg-coral shadow-btn` actions. **Sign out** → `border-border bg-surface shadow-card text-error` (destructive semantics). **Admin section**: `font-display` dropped, Settings icon + Admin badge → `text-coral`/`bg-coral`. **AdminErrorBoundary** → error tokens + `bg-coral` retry.
  - **All 5 modals** (customer/contact/address/dates/invite): scrims `bg-black/40 backdrop-blur-*` → `bg-ink/45` (NO blur — Phase-1 AuthSheet/ChatBot pattern); sheets `glass-strong`/`glass-deep shadow-2xl` → `bg-surface border-t border-border shadow-float rounded-t-[28px]`; headings de-`font-display`-ed; close buttons neutral `bg-ink-50 text-ink-300`; every input/select/textarea `border-ink-50/ink-10 bg-white/bg-cream` → `border-border bg-surface` + coral focus ring; locate buttons → `bg-secondary text-coral`; error texts `text-red-500` → `text-error`; delete/set-default small buttons → `bg-error/10 text-error` / `bg-ink-50 text-ink-300`; how-it-works box → `bg-secondary`; copy-feedback check → `text-success`; primary sheet CTAs get `shadow-btn`. Empty address/date lists are now soft bordered `bg-ink-50` panels instead of bare text.
- **`src/components/AdminPanel.tsx`** (admin-only — **zero logic changes**, verified: only classes/strings/destructuring touched):
  - **PIN gate**: scrim `bg-black/60 backdrop-blur-sm` → `bg-ink/60`; card `glass-strong` → `bg-surface border-border shadow-card rounded-[24px]`; lock icon now in `bg-secondary text-coral` circle; wrong-PIN → `border-error`/`text-error`; Enter `bg-coral shadow-btn`.
  - **Chrome**: root `bg-cream` → `bg-bg`; embedded wrapper gets `border-border rounded-[24px] shadow-card`; top bar `bg-ink` → **solid `bg-coral` brand bar**; tabs strip `bg-white border-ink/8` → `bg-surface border-border`, active tab `bg-coral shadow-btn`; badge `bg-red-500` → `bg-error`.
  - **Surfaces** (bulk sweep): all 20+ `bg-white rounded-2xl` cards → `rounded-[20px] border-border bg-surface shadow-card`; edit forms keep their `border-2 border-coral/30` highlight + gain `shadow-card`; all 28 inputs `border-ink/10 bg-cream/bg-white` → `border-border bg-surface`; sub-panels `bg-cream` → `bg-ink-50`; row dividers/thumb borders → `border-border`.
  - **Semantic colors**: in-stock `emerald` → `bg-success/10 text-success`; WhatsApp-customer buttons `green` → success tokens; destructive `bg-red-50/500 text-red-400/500` → `bg-error/10`/`bg-error`/`text-error`; off-palette "Approved/Hidden" blue/orange → `bg-coral-100 text-coral-700` / `bg-ink-100 text-ink-300`; review stars `fill-amber-400` → `fill-gold text-gold` (Phase-0 soft amber); premium tier `bg-gradient-to-r from-amber-400 to-amber-500` → **solid `bg-gold`**; "Advance proof" amber chip → `bg-gold-light text-ink`.
  - **Gradients/blur stripped**: gallery caption overlay `bg-gradient-to-t from-black/60` → solid `bg-ink/70`; lightbox scrim → `bg-ink/90` (no blur); cancel modal scrim → `bg-ink/60`, card → `bg-surface border-border shadow-card-hover`, textarea gets coral focus ring. `EMPTY_BANNER.color` default `#F3E4D0` (cocoa) → `#FFD6E4` (accent).
  - **Empty states**: orders/gallery/reviews/customers tabs + dashboard mini-notes upgraded from bare `text-ink/30` text to designed soft states (`bg-secondary coral-icon circle + headline + subtext` in `bg-surface border-border shadow-card` cards). `font-display` dropped from PIN heading.
  - **Pre-existing baseline errors cleared while restyling** (Phase-4 convention): 7× `TS18046: 'r' is of type 'unknown'` in the reviews tab fixed via one `(r: any)` cast; 1× `TS6133: 'approveReview' unused` fixed by dropping it from the `useReviews()` destructure (commented — reviews are auto-approved, it was never called).
- **`src/screens/AdminScreen.tsx`** — **verify only, unchanged**: thin `<AdminPanel onClose/>` wrapper with no styling of its own. Confirmed the `admin` route hides BottomTabBar (`showTabBar` gates on `view.name === 'tabs'`) and renders above QuickBar (z-70 > z-45).

### Fixed-overlay / gesture cross-check
- Profile (tab screen): header now reserves `pr-18` right clearance for QuickBar (z-45); all 5 Profile modals scrims sit at z-50/z-60/z-80 → above QuickBar; BottomTabBar (z-100) already auto-hides on every one of them via the existing `useModalDepth` hooks (unchanged).
- AdminPanel full-screen mode z-70 above QuickBar; lightbox + cancel-reason modal z-200 topmost. Splash: no global chrome renders on `splash` route (App.tsx gate, unchanged).
- **No swipe/drag code exists in any Phase-5 file** (grep: `touchstart|touchmove|pointerdown|onPointer` = 0 hits) — Cart/Product native-touch gestures untouched this phase.

### Verification (self)
- `npx tsc --noEmit`: **✓ Verified.** Baseline (`/tmp/baseline_tsc.txt` = 68 lines, captured from clean tree before edits) → 59 lines. `diff` shows the ONLY change is removal of the 9 pre-existing AdminPanel errors listed above. **Zero new errors.**
- `npm run build`: **✓ Passed** (Vite production bundle built in **5.35s**).
- `grep -rn` across all 4 touched files for `glass-strong|glass-deep|backdrop-blur|font-display|font-brand|Fraunces|Great Vibes|linear-gradient|bg-gradient|bg-black/|#A8672E|#2A1B12|#C9963C|#5C3A22|#3D2418|#FBF6EF|#E8C68F|#D9A85E|#6E2A45|#5F7556|#EEE1D2|#FDF8F1|#F3E4D0|#6B3A18` → **zero matches**. Fully clean.
- Remaining `bg-white` hits in AdminPanel are intentional: 2 toggle-switch knobs + lightbox image/close-button chrome on the dark scrim. `package-lock.json` platform-metadata churn from `npm install` was reverted (`git checkout --`), so the phase contains only the 4 intended files.

### Handoff / pending for Phase 6
- **NEXT = Phase 6 (final consistency pass, ALL screens)** — run only now that 0–5 are all done. Checklist per brief: side-by-side spacing/radius/shadow/type audit across every screen; `grep -rn` for leftover stragglers (`glass-*`, `backdrop-blur`, `Fraunces`, `Great Vibes`, inline cocoa hexes, `bg-gradient`, `font-display`/`font-brand`) across `src/` — expected remaining hits: **`LocationGate.tsx` (3 inline cocoa hexes)** — out-of-scope business logic per the brief; Phase 6 should decide whether to do the "trivial quiet swap to `bg-bg`/`text-ink`" Phase-0 allowed, or document leaving it. `src/components/WishlistScreen.tsx` dead duplicate also still holds old classes — confirm it's still unrouted before touching.
- Visible brand-identity notes from this phase: the Profile identity card lost its dark block (now white surface + coral avatar), and the Wallet card is now brand pink (previously dark cocoa). Both are deliberate, flagged for Buddy review.
- 8px-grid note: sheets now `rounded-t-[28px]`, cards `rounded-[20px]`, hero/profile card `rounded-[24px]` — inside the brief's 20–24px band (28px on sheet tops mirrors BottomTabBar/adjacent sheet geometry already in the app).
