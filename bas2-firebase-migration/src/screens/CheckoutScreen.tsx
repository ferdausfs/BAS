...
export default function CheckoutScreen({ onBack }: Props) {
  ...
  const uploadPaymentScreenshot = async (): Promise<string | undefined> => {
    ...
    // Firebase Storage upload is handled in the hook (useOrders). 
    // For now, fallback to base64 for compatibility (same as before).
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Profile sync moved to Firebase (useAuth). Keeping behavior intact.
