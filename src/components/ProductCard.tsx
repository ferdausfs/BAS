...
export default function ProductCard({ product, wished, onOpen, onWish, variant = 'horizontal' }: Props) {
  ...
  return (
    <article onClick={onOpen} className={cardClass}>
      ...
      {/* Reference-B style: no inner details card, only shadow/gradient over the image. */}
      <span className="pointer-events-none absolute inset-0 z-[2] bg-gradient-to-b from-black/0 via-black/5 to-black/70" />
      ...
      {!zoomed && (
        <div className="absolute inset-x-0 bottom-0 z-[3] px-3.5 pb-3.5 text-white [text-shadow:0_2px_12px_rgba(0,0,0,.44)]">
