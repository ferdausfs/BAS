...
export default function CouponsScreen() {
  ...
  return (
    <div className="flex h-full flex-col bg-[#FBF6EF]">
      {/* Header */}
      <header className="flex flex-shrink-0 items-center gap-3 px-5 pt-14 pb-3">
        <button
          onClick={back}
          className="flex h-9 w-9 items-center justify-center rounded-full bg-white text-ink transition active:scale-90"
        ...
        <div>
          ...
          <h1 className="font-display text-[20px] font-bold tracking-tight text-ink">My Coupons</h1>
        </div>
      </header>
      ...
      <div className="no-scrollbar flex-1 overflow-y-auto px-5 pb-10">
        {coupons.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-24 text-center">
            <div className="flex h-16 w-16 items-center justify-center rounded-full bg-ink-50 text-ink-200">
              <Ticket size={28} strokeWidth={1.5} />
            </div>
            <p className="mt-3 text-[14px] font-medium text-ink-300">No coupons available right now</p>
            ...
            {coupons.map((c) => {
              ...
              return (
                <div
                  ...
                  style={{ boxShadow: '0 1px 2px rgba(26,19,17,.03), 0 10px 28px -14px rgba(26,19,17,.18)' }}
                >
                  <div className="flex">
                    {/* Discount badge strip */}
                    <div
                      className="flex w-[76px] flex-shrink-0 flex-col items-center justify-center gap-0.5 py-4 text-white"
                      style={{ background: 'linear-gradient(160deg, #E8526A 0%, #A8672E 100%)' }}
                    >
                      <span className="font-display text-[20px] font-bold leading-none">{c.discount}%</span>
                    ...
                    <div className="flex-1 p-3.5">
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="font-display text-[14px] font-bold tracking-wide text-ink">{c.code}</span>
                      </div>
                      ...
                      <div className="mt-2 flex items-center justify-between">
                        ...
                        <button
                          ...
                          className={`flex items-center gap-1 rounded-full px-3 py-1.5 text-[10.5px] font-bold transition active:scale-95 ${
                            copiedId === c.id ? 'bg-emerald-50 text-emerald-600' : 'bg-coral/10 text-coral'
                          }`}
                        >
                          {copiedId === c.id ? (
                            <>
                  ...
                  <span className="absolute left-[70px] top-1/2 h-3.5 w-3.5 -translate-x-1/2 -translate-y-1/2 rounded-full bg-[#FBF6EF]" />
                  <span className="absolute left-[70px] bottom-0 h-3.5 w-3.5 -translate-x-1/2 translate-y-1/2 rounded-full bg-[#FBF6EF]" />
                </div>
              );
            })}
          </div>
